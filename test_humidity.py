...
class humidity_test():
    ...
    def plot_equipment_category(self):
        ...
        temp_time = [0,2,8,24]
        temperature_levels = [self.temp_sp1, self.temp_sp2, self.temp_sp2,self.temp_sp1] 
        humidity_time = [0,2,8,24]
        humidity_levels = [self.relative_humidity_sp1, self.relative_humidity_sp2, self.relative_humidity_sp2, self.relative_humidity_sp1]
        ...
        plt.savefig('humidity_test.jpg')
        plt.close()
def humidity_test_active(input = ''):
    humidity_test(input).set_category_variables()  # this needs to change to accomindate different inputs A,B or C.
    humidity_test(input).plot_equipment_category()
    return 'worked'
# humidity_test_active =  humidity_test()
# humidity_test_active().set_category_variables()  #this needs to change to accomindate different inputs A,B or C.
